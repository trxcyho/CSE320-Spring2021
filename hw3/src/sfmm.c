...
void *sf_realloc(void *pp, size_t rsize) {
	...
	if(original_size >= updated_size){
		...
		if(original_size - updated_size < 32)
			...
			sf_block *body = (void *)realloc_block + original_size;
			body -> header = body -> header & ~PREV_BLOCK_ALLOCATED;
			*(sf_header *)((char *)body + ((~0x3 & body -> header) - 8)) = body -> header;
			return ((sf_block *)sf_split(realloc_block, updated_size)) -> body.payload;
		}
	}
...
void *sf_memalign(size_t size, size_t align) {
	//make sure align at least 32
	if(align < 32){
		sf_errno = EINVAL;
		return NULL;
	}
	//make sure align is a power of 2
	size_t updated_size = align;
	...
	sf_block *new_block = (void *)sf_malloc(size + align + 40) -8;
	if(new_block == NULL){
		sf_errno = ENOMEM;
		return NULL;
	}
	...
	*(sf_header *)((char *)new_block + ((~0x3 & new_block -> header) - 8)) = new_block -> header;//set footer of new block
	...
	//coalease new block?
    return sf_realloc((void *)aligned_block + 8, size);
}
...
sf_block *sf_split(sf_block *block, size_t size){
	...
	*(sf_header *)((char *)split_block + ((~0x3 & split_block -> header) - 8)) = split_block -> header;
	...
	//coalesce with nearby blocks and add
