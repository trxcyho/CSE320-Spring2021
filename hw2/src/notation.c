...
#endif
{
  ...
  case DEFAULTP:
    ...
    break;
  case TITLE:
  //TODO: 0 was NULL for 1486, 1489, 1492
...
#endif
{
 ...
 //      if ((infile = fopen (cp,"r")) == NULL)
  ...
  if ((optind < argc) &&(infile = fopen(argv[optind], "r")) == NULL)
    fatal((stderr, "can't open %s input file\n", argv[optind]));
  if (infile != NULL && (optind + 1)< argc)
    fatal((stderr, "too many files\n"));
  return(argc);
}
...
#endif
{
  ...
  if (error_flag) {
    // free(dr); //free for when error flag occurs
    error((stderr,"\nLast valid position:\n"));
    output_board(dr,tos);
    fatal((stderr,"\nToo many errors"));
  }
